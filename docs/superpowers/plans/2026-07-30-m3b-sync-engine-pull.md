...
Reparent safety needs NO guard inside `pullOnce`: the inner loop can only exit with `dirty === false`, so a reparent abort (which sets `dirty`) is always re-drained BEFORE any pull — a pull can never observe a half-rewritten loser id, and the loser id never exists server-side. (Do NOT add a `dirty` check in `pullOnce` — it would be unreachable dead code.) `pullOnce()` skips (publishing nothing) when: `deps.pull` undefined, `deps.sessionUserId` null/undefined (pull unarmed until a session exists), or the just-published cycle-end state has `online === false` AND `pending > 0` — i.e. the drain provably starved offline mid-queue THIS cycle (the offline stop is visible via `getState()` — `runOnce` stays untouched). The `pending > 0` conjunct is load-bearing: `online: false` alone may be STALE from a previous cycle's pull-offline outcome, and with an empty queue nothing on the push side ever republishes `online: true` — a skip gate on bare `online === false` would therefore skip every retried pull forever and deadlock the arm-2 ladder. With `pending === 0` the pull ALWAYS runs, even from an `online: false` state, and acts as the connectivity probe itself (cheap: the orchestrator short-circuits on the first offline-classified failure — no radio-burn). Otherwise: publish `{...state, syncing: true}`; `await deps.pull({sessionUserId})` inside try/catch (a throw is treated as `{ok: false, offline: false, error: message}` — never-rejects stays absolute); end-publish folds the outcome — `completedPulls` bumps by 1 IFF `outcome.ok` (engineApi.ts's documented contract is "bumped after every successful pull" — `ok` also covers Tier-1 replaces and sweep deletions that per-feed row counts would miss; monotone counter beside `reparentsCount`; replaces BOTH hardcoded `0`s at ~:117/~:235), `outcome.ok` → ALSO `online: true` (a successful pull proves the link — this is the ONLY recovery path that clears a pull-induced `online: false` when the queue stays empty), `outcome.offline` → `online: false, lastError: null` (never-alarm); otherwise `lastError` = the drain's lastError when non-null, else `outcome.error` (push failures always win; a pull failure lands only when the push phase was clean).
...
- [ ] Failing engineCore tests: pull runs after clean drain; skipped when the drain stopped offline mid-queue (cycle-end `online: false` AND `pending > 0`) / no dep / null sessionUserId; pull STILL runs when `online: false` but `pending === 0` (stale pull-induced offline must not deadlock the retry ladder); `ok` pull outcome republishes `online: true` (the recovery pair: cycle N pull-offline → cycle N+1 empty-queue pull attempted → success flips online back); enqueue-during-pull re-drains then re-pulls; reparent abort re-drains before any pull (assert push call order: create → reparent-abort → re-drain pushes → THEN pull — proving the loop structure, not a guard); `completedPulls` bumps IFF `outcome.ok` (an `ok: false` outcome with feeds partially applied does NOT bump), monotone across cycles; push lastError not masked by pull error; pull offline → `online: false, lastError: null`; pull dep throwing → cycle still publishes end-state (never rejects). ALL existing engineCore tests pass UNMODIFIED (pin holds).
